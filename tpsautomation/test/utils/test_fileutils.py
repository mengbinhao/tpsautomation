...
def test_if_file_or_dir_exists():
    ...
    assert fu.FileUtils.is_file_or_dir_exists(
        r'C:\Users\T5810\Desktop\tpsautomation\bin_noexist') is not True
    assert fu.FileUtils.is_file_or_dir_exists(
        r'C:\Users\T5810\Desktop\tpsautomation\TODO_noexist.md') is not True


def test_read_file_by_line():
    assert len(fu.FileUtils.read_file_by_line(
        r'C:\Users\T5810\Desktop\tpsautomation\pytest.ini')) > 0


@pytest.mark.xfail
def test_write_file_by_line():
    assert len(fu.FileUtils.read_file_by_line(
        r'C:\Users\T5810\Desktop\tpsautomation\pytest_noexist.ini')) == 0
...
def test_read_file_last_line():
    assert fu.FileUtils.read_small_file_last_line(
        r'C:\Users\T5810\Desktop\tpsautomation\logs\myapp.log').find(r'pyautoguiwrapper.py') != -1

    with pytest.raises(FileNotFoundError) as excinfo:
        fu.FileUtils.read_small_file_last_line(
            r'C:\Users\T5810\Desktop\tpsautomation\logs\myapp_notexists.log')
    assert excinfo.type.__name__ == 'FileNotFoundError'


@pytest.mark.skip
def test_get_case_list():
    ret = fu.FileUtils.get_case_list(
        r'C:\Users\T5810\Desktop\tpsautomation\tpsautomation\testcases\beihang\toudao')
    assert len(ret) == 4
