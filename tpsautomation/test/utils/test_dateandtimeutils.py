...
def test_get_date_and_time_as_datetime():
    str_val = '2018-01-01 16:26:22'
    assert isinstance(du.DateAndTimeUtils.get_date_and_time_as_datetime(
        str_val), cv.ConstValue.DATETIME_DATETIME_FOR_TYPE)

    with pytest.raises(ValueError) as excinfo:
        du.DateAndTimeUtils.get_date_and_time_as_datetime(str_val, 'wfwfw')
    assert excinfo.type.__name__ == 'ValueError'

    with pytest.raises(TypeError) as excinfo:
        du.DateAndTimeUtils.get_date_and_time_as_datetime(str_val, {})
...
def test_convert_seconds_to_display_time():
    ...
    assert du.DateAndTimeUtils.convert_seconds_to_display_time(
        3661) == '1 hours, 1 min, 1 sec'
    assert du.DateAndTimeUtils.convert_seconds_to_display_time(
        90061) == '1 days, 1 hours, 1 min, 1 sec'
